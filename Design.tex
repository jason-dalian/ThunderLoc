...
used by the participants of the thunder can be shown in
Fig. \ref{Screen}. Figure \ref{Dual}. shows the two channel sound signals from dual-microphone in a mobile phone.
It is easy to see that the thunder firstly reaches the microphone that corresponding to the top channel.
  ...
  \begin{figure}[ht]
     ...
     \vspace{2mm}
            \caption{The diagram of signal processing on client end.}
            \label{Diagram}
            \vspace{-4mm}
  \end{figure}
...
Figure \ref{Diagram} depicts the flow of the ThunderLoc client application. 
...
In this section, we introduce the basic localization method in ThunderLoc system. 
Let us consider the 4G network with $N$ dual-microphone smartphones randomly deployed in 2D area. 
The position and orientation of a smartphone are assumed to be known, which can be estimated by its GPS(or Wi-Fi based indoor localization) and motion sensors, respectively.
...
We propose a binary sensor model by utilizing the sign of TDOA as 1-bit measurement information.
The TDOA of dual-channel acoustic signals is quantized to 1-bit data, which reveals the left/right information about the direction of the incoming Thunder.
Using 1-bit measurement information allows for inexpensive sensing as well as minimal communication load.
Moreover, the binary left/right sensing model is robust to position errors and orientation errors of smartphones.
As shown in Fig. \ref{overview}, for each dual-microphone smartphone node, 
TDOA of dual-microphone is computed by the conventional time delay estimation method (such as generalized cross correlation, GCC), % ~\cite{knapp1976generalized}
then we can get the binary sequence of the thunder. ThunderLoc system estimates the position of thunders by processing the binary sequence.
...
For two faces $f_i$ and $f_j$ in Fig. \ref{Hamming}, there are two types
of measurement distance: (i) the geographical distance $GD({f_i},{f_j})$ between the center points of ${f_i}$ and ${f_j}$, and (ii) the Hamming distance
$HD({f_i},{f_j})$ between the corresponding binary sequences of ${f_i}$ and ${f_j}$. Hamming distance measures the number of dimensions where two vectors have different values. 
...
(1) Dividing the space into $p$ grid points : Suppose there is an acoustic source at the point $P{_i},i=1,...,p$, 
the $j-th$ smartphone $n{_j},j=1,...,N$, can compute the TDOA and get the sign of the TDOA.
...
As shown in Fig. \ref{Impact}$(a)$, when the measurement binary data from node B is flipped, the binary sequence $'10'$ is turned into $'11'$, reducing the accuracy of localization system. 
In Fig. \ref{Impact}$(b)$, if the position or orientation of sensor B exists some measurement error, 
...
\end{equation}
where $\sigma$ is the parameter to decide the weighting strategy, and $N$ is the number of smartphones in ThunderLoc. 
