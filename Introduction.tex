...
%~\cite{arechiga2011acoustic}.
With the recent advances in mobile computing and communication technology, most mobile devices (e.g. smart phones, tablets and laptops) are equipped with one/multiple microphones onboard.
Wireless acoustic sensor networks (WASNs) can overcome these limitations. 
Thunder localization in WASN is becoming feasible due to recent advances in personal portable computing devices with the rapid deployment ability.
...
ThunderLoc constructs the binary sequence table that maps all these feasible binary sequences to the corresponding regions by using the locations and directions information of the smartphone nodes.
